...
#[async_trait]
impl FramedBinding<IpAddress> for UdpBinding<'_> {
...
impl Stream for UdpBinding<'_> {
    type Item = Result<(Packet, IpAddress), (TransportError, Option<IpAddress>)>;
    ...
    fn poll_next(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        ...
        match Packet::from_bytes(&recv_buf[..n]) {
            Ok(p) => Poll::Ready(Some(Ok((p, ep.addr)))),
            Err(e) => {
...
impl Sink<(Packet, IpAddress)> for UdpBinding<'_> {
    ...
    fn start_send(mut self: Pin<&mut Self>, item: (Packet, IpAddress)) -> Result<(), Self::Error> {
        // TODO: error handling
    ...
    fn poll_flush(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Result<(), Self::Error>> {
...

        {
            ...
            let fut = socket.send_to(&packet, (target_u, 9401));
            futures_util::pin_mut!(fut);
            match fut.poll(cx) {
                Poll::Ready(Ok(())) => {}
                Poll::Ready(Err(_)) => {
